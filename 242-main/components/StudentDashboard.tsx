...
import { Layout, Gift, Sparkles, Megaphone, Lock, BookOpen, AlertCircle, Edit, Settings, Play, Pause, RotateCcw, MessageCircle, Gamepad2, Timer, CreditCard, Send, CheckCircle, Mail, X, Ban, Smartphone, Trophy, ShoppingBag, ArrowRight, Video, Youtube, Home, User as UserIcon, Book, BookOpenText, List, BarChart3, Award, Bell, Headphones, LifeBuoy, WifiOff, Zap, Star, Crown, History, ListChecks, Rocket, Ticket, TrendingUp } from 'lucide-react';
...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings, onStartWeeklyTest, activeTab, onTabChange, setFullScreen, onNavigate, isImpersonating, onNavigateToChapter }) => {
     ...
     if (evt?.enabled) {
         const interval = setInterval(() => {
             ...
             if (now < startsAt) {
                 ...
                 // AUTO START WHEN COOLDOWN HITS 0
                 if (diff <= 1000) {
      ...
      if (activeTab === 'HOME') {
          ...
          return (
              <div className="space-y-6 pb-24">
                ...
                <div className="mx-1 bg-gradient-to-br from-slate-900 to-slate-800 p-6 rounded-3xl shadow-2xl border border-white/10 relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div
                                ...
                                className="bg-white/5 backdrop-blur-md p-4 rounded-2xl border border-white/10 cursor-pointer hover:bg-white/10 transition-all group"
                            >
                                ...
                                {user.mcqHistory && user.mcqHistory.length > 0 ? (
                                    <div className="flex items-end justify-between h-20 gap-1 px-1">
                                        {user.mcqHistory.slice(-10).map((h, i) => {
                                            const score = h.totalQuestions > 0 ? Math.round((h.correctCount / h.totalQuestions) * 100) : 0;
                                            return (
                                                <div key={i} className="flex-1 flex flex-col items-center gap-1 group relative h-full justify-end">
                                                    {/* Tooltip for Topic Name */}
                                                    <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[8px] font-black px-2 py-1 rounded shadow-xl opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 pointer-events-none max-w-[120px] truncate">
                                                        {h.chapterTitle || 'Test'} ({score}%)
                                                    </div>
                                                    <div
                                                        className={`w-full rounded-t-sm transition-all duration-500 ${score >= 80 ? 'bg-green-500' : score >= 50 ? 'bg-blue-500' : 'bg-red-500'}`}
                                                        style={{ height: `${score}%` }}
                                                    />
                                                </div>
                                            );
                            ...
                            >
                                <div className="flex justify-between items-center mb-3">
                                    ...
                                    <ArrowRight size={12} className="text-white/20 group-hover:text-purple-400 transition-colors" />
                                </div>
                                {user.topicStrength && Object.keys(user.topicStrength).length > 0 ? (
                                    <div className="space-y-2">
                                        {Object.entries(user.topicStrength).slice(0, 3).map(([topic, stats]) => {
                  ...
                  <div className="bg-gradient-to-r from-pink-50 to-rose-50 p-4 rounded-2xl border border-pink-100 shadow-sm mt-4">
                      ...
                      <div>
                          ...
                          <div className="grid grid-cols-3 gap-3">
                              {/* Row 1 */}
                              <button onClick={() => setShowInbox(true)} className="h-16 bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform relative group">
                              ...
                              <button onClick={() => onTabChange('ANALYTICS')} className="h-16 bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                                  <BarChart3 size={18} style={{ color: 'var(--primary)' }} />
                                  <span className="text-[10px] font-black text-slate-700 uppercase tracking-wider">Analytics</span>
                              </button>

                              <button onClick={() => onTabChange('HISTORY')} className="h-16 bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                                  <History size={18} style={{ color: 'var(--primary)' }} />
                                  <span className="text-[10px] font-black text-slate-700 uppercase tracking-wider">History</span>
                              </button>

                              {/* Row 2 */}
                              <button onClick={() => onTabChange('STORE')} className="h-16 bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                              ...
                              <button onClick={() => onTabChange('SUB_HISTORY' as any)} className="h-16 bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                                  <CreditCard size={18} style={{ color: 'var(--primary)' }} />
                                  <span className="text-[10px] font-black text-slate-700 uppercase tracking-wider">My Plan</span>
                              </button>

                              {isGameEnabled && (
                                <button onClick={() => onTabChange('GAME')} className="h-16 bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                              ...
                              {/* Row 3 - Full Width or centered items if needed, keeping grid compact */}
                              <button onClick={() => onTabChange('REDEEM')} className="h-16 bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                              ...
                              <button onClick={() => onTabChange('PRIZES')} className="h-16 bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                                  <Award size={18} style={{ color: 'var(--primary)' }} />
                                  <span className="text-[10px] font-black text-slate-700 uppercase tracking-wider">Prizes</span>
                              </button>

                              <button onClick={() => onTabChange('LEADERBOARD')} className="h-16 bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
