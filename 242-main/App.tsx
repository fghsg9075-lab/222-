...
import { getChapterData, saveChapterData, checkFirebaseConnection, saveTestResult, saveUserToLive, updateUserStatus, getUserData, subscribeToSettings, auth, savePublicActivity } from './firebase';
  ...
  const handleContentGeneration = async (type: ContentType, count?: number, forcePay: boolean = false) => {
    ...
    if (onlineContent) {
        if (type === 'PDF_FREE' && onlineContent.freeLink) {
            ...
            foundAdminContent = true;
        } else if (type === 'AUDIO' && (onlineContent.audioPlaylist?.length > 0 || onlineContent.freeAudioLink || onlineContent.premiumAudioLink)) {
            // Prioritize Playlist -> Premium Link -> Free Link
            const audioUrl = onlineContent.premiumAudioLink || onlineContent.freeAudioLink || (onlineContent.audioPlaylist && onlineContent.audioPlaylist.length > 0 ? onlineContent.audioPlaylist[0].url : '');
            const audPrice = onlineContent.audioCreditsCost !== undefined ? onlineContent.audioCreditsCost : 0;
            onlineContent = { ...onlineContent, content: audioUrl, audioPlaylist: onlineContent.audioPlaylist, type, price: audPrice };
            foundAdminContent = true;
        } else {
